...
**Process, gates, CWD, multi-agent guidance:** [`../../docs/working-on-plato.md`](../../docs/working-on-plato.md).
...
- `stdlib/` — **Forward stdlib vocabulary** (ex-`plato-src-v3`). New *vocabulary* goes here.
  ...
  file, and the folder is **flat** — no subfolders.
...
- `parakeet/` — sub-submodule, PRE-EXISTING DIRTY STATE. Never touch, never stage. Grammar changes
  there are pushed manually, on purpose.
...
- `.\tools\check-stdlib-fast.ps1` — the forward-stdlib inner loop (seconds). Two gates:
  ...
  (`ForwardStdLibDiagnosticCountDoesNotRegress` in `PlatoTests/ForwardStdLibCheckerTests.cs`) —
