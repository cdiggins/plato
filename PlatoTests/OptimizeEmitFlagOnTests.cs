...
{
    /// <summary>
    /// Optimizer-mode (<c>--optimize</c>) counterpart of <see cref="EmitFlagOnTests"/>:
    /// generates the WHOLE optimize-mode library twice — once with <see cref="CSharpWriter.UseTir"/>
    ...
    /// flip criterion for retiring the legacy writer's optimize-mode heuristics.
    /// </summary>
    [TestFixture]
    public static class OptimizeEmitFlagOnTests
    {
        ...
        public static void UseTirOnReproducesTheFullOptimizedLibraryByteForByte()
        {
            ...
            Assert.AreEqual(0, flagOn.TirFallbackBodies,
                "an optimize-mode body fell back to the legacy writer");

            Assert.IsEmpty(differing,
                "UseTir=true changed optimize-mode output vs UseTir=false");
        }
    }
}
