...
namespace Plato.Math
{
    public partial struct Box : ITransformable3D<Box>
    {
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Box Create(IEnumerable<Vector3> points = null)
        {
        ...
        public Box Scale(float scale)
            => new Box(Recenter().Min * scale, Recenter().Max * scale).Translate(Center);
        ...
        public Box SetCenter(Vector3 v)
            => FromCenterAndExtent(v, Extent);

        public Box SetExtent(Vector3 v)
            => FromCenterAndExtent(Center, v);

        public static Box FromCenterAndExtent(Vector3 center, Vector3 extent)
            => new Box(center - extent / 2f, center + extent / 2f);

        public Box Transform(Matrix4x4 mat)
