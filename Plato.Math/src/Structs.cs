...
namespace Plato.Math
{
        ...
        , IComparable< Vector2D >
    {
        ...
        public static Vector2D Create((double x, double y) tuple) => new Vector2D(tuple);
        public override bool Equals(object obj) => obj is Vector2D x && Equals(x);
        public override int GetHashCode() => Hash.Combine(X.GetHashCode(), Y.GetHashCode());
        public override string ToString() => $"Vector2D(X = {X}, Y = {Y})";
        public void Deconstruct(out double x, out double y) {x = X; y = Y; }
        public bool Equals(Vector2D x) => X == x.X && Y == x.Y;
        public static bool operator ==(Vector2D x0, Vector2D x1) => x0.Equals(x1);
        public static bool operator !=(Vector2D x0, Vector2D x1) => !x0.Equals(x1);
        public static implicit operator Vector2D((double x, double y) tuple) => new Vector2D(tuple);
        public static implicit operator (double x, double y)(Vector2D self) => (self.X, self.Y);
        public bool AlmostEquals(Vector2D x, float tolerance = Constants.Tolerance) => X.AlmostEquals(x.X, tolerance) && Y.AlmostEquals(x.Y, tolerance);
        public static Vector2D Zero = new Vector2D(default, default);
        ...
        public Vector2D WithY(double x) => new Vector2D(X, x);
        public static Vector2D operator +(Vector2D value1, Vector2D value2) => new Vector2D(value1.X + value2.X,value1.Y + value2.Y);
        public static Vector2D operator +(Vector2D value1, double value2) => new Vector2D(value1.X + value2,value1.Y + value2);
        public static Vector2D operator +(double value1, Vector2D value2) => new Vector2D(value1 + value2.X,value1 + value2.Y);
        public static Vector2D operator -(Vector2D value1, Vector2D value2) => new Vector2D(value1.X - value2.X,value1.Y - value2.Y);
        public static Vector2D operator -(Vector2D value1, double value2) => new Vector2D(value1.X - value2,value1.Y - value2);
        public static Vector2D operator -(double value1, Vector2D value2) => new Vector2D(value1 - value2.X,value1 - value2.Y);
        public static Vector2D operator *(Vector2D value1, Vector2D value2) => new Vector2D(value1.X * value2.X,value1.Y * value2.Y);
        ...
        public static Vector2D UnitX = Zero.WithX(1.0);
        public static Vector2D UnitY = Zero.WithY(1.0);
        public Vector2D(double value) : this(value, value) { }
        public static Vector2D operator -(Vector2D value) => Zero - value;
        public static double Dot(Vector2D value1, Vector2D value2) => value1.X * value2.X + value1.Y * value2.Y;
        public double Dot(Vector2D value) => Vector2D.Dot(this, value);
        public bool AlmostZero(float tolerance = Constants.Tolerance) => X.Abs() < tolerance && Y.Abs() < tolerance;
        public bool AnyComponentNegative() => MinComponent() < 0;
        public double MinComponent() => (X).Min(Y);
        public double MaxComponent() => (X).Max(Y);
        ...
        public bool IsNaN() => X.IsNaN() || Y.IsNaN();
        public bool IsInfinity() => X.IsInfinity() || Y.IsInfinity();
        public int CompareTo(Vector2D x) => (MagnitudeSquared() - x.MagnitudeSquared()).Sign();
        public static bool operator <(Vector2D x0, Vector2D x1) => x0.CompareTo(x1) < 0;
        public static bool operator <=(Vector2D x0, Vector2D x1) => x0.CompareTo(x1) <= 0;
        public static bool operator >(Vector2D x0, Vector2D x1) => x0.CompareTo(x1) > 0;
        public static bool operator >=(Vector2D x0, Vector2D x1) => x0.CompareTo(x1) >= 0;
    }
