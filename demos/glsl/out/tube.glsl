...
// Created on 2026-07-19 10:07:03 PM
...
float Extrude(vec3 p, int segs, float r);
...
vec3 Path(float u)
{
    float a = (u * 6.283185307179586);
    float x = (CosR(a) + (0.5 * CosR((3.0 * a))));
    float z = (SinR(a) - (0.5 * SinR((3.0 * a))));
    float y = (0.55 * SinR((2.0 * a)));
    return vec3(x, y, z);
}
...
float Extrude(vec3 p, int segs, float r)
{
    float d = 100000.0;
    {
        ...
        {
            {
                float u0 = (ToNumber(i) / ToNumber(segs));
                float u1 = (ToNumber((i + 1)) / ToNumber(segs));
                d = Min(d, SdCapsule(p, Path(u0), Path(u1), r));
            }
            i = (i + 1);
        }
    }
