// Autogenerated file: DO NOT EDIT
// Created on 2026-07-30 12:39:08 AM

using System.Runtime.CompilerServices;
using System.Runtime.Serialization;
using System.Runtime.InteropServices;
using static System.Runtime.CompilerServices.MethodImplOptions;
using Ara3D.Collections;

namespace Ara3D.Geometry
{
    [DataContract, StructLayout(LayoutKind.Sequential, Pack=1)]
    public partial struct Line2D: IArrayLike<Line2D, Point2D>, IInterval<Point2D>
    {
        // Fields
        [DataMember] public readonly Point2D A;
        [DataMember] public readonly Point2D B;

        // With functions 
        [MethodImpl(AggressiveInlining)] public Line2D WithA(Point2D a) => new Line2D(a, B);
        [MethodImpl(AggressiveInlining)] public Line2D WithB(Point2D b) => new Line2D(A, b);

        // Regular Constructor
        [MethodImpl(AggressiveInlining)] public Line2D(Point2D a, Point2D b) { A = a; B = b; }

        // Static factory function
        [MethodImpl(AggressiveInlining)] public static Line2D Create(Point2D a, Point2D b) => new Line2D(a, b);

        // Static default implementation
        public static readonly Line2D Default = default;

        // Implicit converters to/from value-tuples and deconstructor
        [MethodImpl(AggressiveInlining)] public static implicit operator (Point2D, Point2D)(Line2D self) => (self.A, self.B);
        [MethodImpl(AggressiveInlining)] public static implicit operator Line2D((Point2D, Point2D) value) => new Line2D(value.Item1, value.Item2);
        [MethodImpl(AggressiveInlining)] public void Deconstruct(out Point2D a, out Point2D b) { a = A; b = B;  }

        // Object virtual function overrides: Equals, GetHashCode, ToString
        [MethodImpl(AggressiveInlining)] public bool Equals(Line2D other) => A.Equals(other.A) && B.Equals(other.B);
        [MethodImpl(AggressiveInlining)] public bool NotEquals(Line2D other) => !(A.Equals(other.A) && B.Equals(other.B));
        [MethodImpl(AggressiveInlining)] public override bool Equals(object obj) => obj is Line2D other ? Equals(other) : false;
        [MethodImpl(AggressiveInlining)] public override int GetHashCode() => Intrinsics.CombineHashCodes(A, B);
        [MethodImpl(AggressiveInlining)] public override string ToString() => $"{{ \"A\" = {A}, \"B\" = {B} }}";

        // Explicit implementation of interfaces by forwarding properties to fields

        // IArrayLike predefined functions
        [MethodImpl(AggressiveInlining)] public int NumComponents() => 2;
        [MethodImpl(AggressiveInlining)] public IReadOnlyList<Point2D> Components() => Intrinsics.MakeArray<Point2D>(A, B);
        [MethodImpl(AggressiveInlining)] public static Line2D CreateFromComponents(IReadOnlyList<Point2D> numbers) => new Line2D(numbers[0], numbers[1]);

        [MethodImpl(AggressiveInlining)] public static Line2D CreateFromComponent(Point2D x) => new Line2D(x, x);

        // Implemented interface functions
        [MethodImpl(AggressiveInlining)] public Vector2 Direction() => new Vector2(this.B.X, this.B.Y).Subtract(new Vector2(this.A.X, this.A.Y));
        [MethodImpl(AggressiveInlining)] public Ray2D Ray2D() => (this.A, new Vector2(this.B.X, this.B.Y).Subtract(new Vector2(this.A.X, this.A.Y)));
        [MethodImpl(AggressiveInlining)]  public static implicit operator Ray2D(Line2D x) => x.Ray2D();
        [MethodImpl(AggressiveInlining)] public Point2D Start() => this.A;
        [MethodImpl(AggressiveInlining)] public Point2D End() => this.B;
        [MethodImpl(AggressiveInlining)] public Point2D Center() => new Point2D(this.A.X.Multiply(1f.Subtract(0.5f)).Add(this.B.X.Multiply(0.5f)), this.A.Y.Multiply(1f.Subtract(0.5f)).Add(this.B.Y.Multiply(0.5f)));
        [MethodImpl(AggressiveInlining)] public Line3D Line3D() => new Line3D(new Point3D(this.A.X, this.A.Y, 0), new Point3D(this.B.X, this.B.Y, 0));
        [MethodImpl(AggressiveInlining)]  public static implicit operator Line3D(Line2D x) => x.Line3D();
        [MethodImpl(AggressiveInlining)] public IReadOnlyList<Point2D> Points() => Intrinsics.MakeArray<Point2D>(this.A, this.B);
        [MethodImpl(AggressiveInlining)] public Point2D At(int n) => this.Components()[n];
        [MethodImpl(AggressiveInlining)] public Point2D Size() => new Vector2(this.B.X, this.B.Y).Subtract(new Vector2(this.A.X, this.A.Y));

        // Unimplemented interface functions
    }
    // Extension methods for the type
    public static class Line2DExtensions
    {
    }
}
