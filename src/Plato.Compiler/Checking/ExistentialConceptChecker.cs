...
{
    /// <summary>
    /// Well-formedness checker for interfaces used in TYPE POSITION (plato-311, Option A). A
    /// self-constrained interface referenced as a field type, return type, or parameter type from a
    /// type that does not implement/inherit it is an EXISTENTIAL reference ("any C") — Plato.CSharpWriter
    /// lowers it to the interface's non-generic object-safe view interface (`interface C`). An interface
    ...
    ///
    /// Diagnostics:
    ///   CHK308 existential (type-position) reference to an interface with an empty object-safe surface
    /// </summary>
    public class ExistentialConceptChecker
    {
        ...
        {
            // FIELDS ONLY, deliberately. A bare interface in a library-function parameter or return
            ...
            // existential. Only an interface-typed FIELD on a concrete type is genuine storage of
        ...
        private void CheckReference(TypeDef owner, TypeExpression te, Symbol origin)
        {
            ...
            if (ConceptGrounding.IsExistentialConceptReference(owner, te) && !te.Def.HasObjectSafeSurface())
            {
                Error("CHK308",
                    ...
                    "least one object-safe member on the interface, or avoid using it in type position",
                    origin);
            }
