// Support machinery for the FORWARD-stdlib conformance suite.
// Adapted verbatim (namespace only) from conformance/Ara3D.SDK.ConformanceTests/ConformanceSupport.cs
// so the two suites share value-generation, manifest, and assert semantics.
// - ValueGen: deterministic, seeded pseudorandom instance construction for generated types.
// - KnownFailures: the quarantine manifest mapping (law-or-witness, type) -> reason + reference.
// - ConformanceRunner: shared invoke/assert logic for law test cases.
...
namespace Plato.ForwardConformanceTests
{
    ...
    public static class ValueGen
    {
        ...
        static bool CanCreate(Type t, int depth, HashSet<Type> inProgress)
        {
            ...
            try
            {
                ...
                if (!ok)
                    Note(t, "no public constructor whose parameters are all constructible");
                return ok;
            }
    ...
    /// <summary>The known-failures manifest: the quarantine queue for forward-stdlib bugs.</summary>
