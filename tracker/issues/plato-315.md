---
id: plato-315
title: Carry legacy geometry interfaces into stdlib: Curve generics, indexed-face access, unwelded face arrays, Procedural doc table; retire or wire Dimensioned
type: problem
status: done
priority: p2
effort: M
...
single-rooted lattice) against the forward `stdlib` geometry interfaces, deciding which legacy
ideas still carry value and closing the question of the rest so it does not get re-litigated.
The new stack (orthogonal capability mixins: `geometry.interfaces` markers +
`geometry-measures.interfaces` + `geometry-queries.interfaces`, `Procedural` unification, first-class
...
dead-interface class needs a decision, and the rest is explicitly left behind.
...
`QuadMesh3D` / `PolygonMesh3D` / `LineSet3D` have no interface-level face access (only
...
- `submodules/Plato/stdlib/meshes.concepts.plato:26-32` — `TriangulatedGeometry3D` is
  ...
  PolygonMesh3D and `meshes-lines-points.plato` LineSet3D have no face-access interface.
...
legacy generalizations were dropped without an explicit decision, and one interface
...
1. **`Curve<TRange>` — generic curve root** (effort S, risk low). Add
   `interface Curve<TRange> inherits Procedural<Number, TRange>` to `curves.concepts.plato`;
   ...
   and stay per-dimension unless the interface grows `where TRange:` bounds (possible — precedent
   `algebra-metric.concepts.plato:49` — but a separate decision; do not add bounds in the first
   pass). File-partitioning: fits `curves.concepts.plato` (currently 8 interfaces, cap 12).
...
2. **Generic indexed-face access** (effort M, risk low). Legacy
   ...
   `interface FaceIndexedGeometry3D<TFace> { FaceCount; FaceAt(x, i): TFace; PositionAt(x, v): Point3D }`
   ...
   new file chose. Extending implemented interfaces obliges every implementor (LIBRARIES.md rule
...
3. **Lift the `procedurals.plato` doc prose — table only** (effort S, risk none). The
   ...
   have named interfaces: ScalarField2D, ImplicitRegion2D, ...). Do NOT port the legacy
...
4. **Unwelded face-array types: `TriangleArray3D`, `QuadArray3D`** (effort S, risk low;
   ...
   **Interfaces to implement:** `Value`, `Geometry3D`, `Meshable3D` (weld to `TriangleMesh3D`),
   ...
   wrong Euler characteristic. A type that cannot satisfy the interface is the design working as
...
5. **`PrimitiveCollection2D/3D<TPrimitive>` — dependent on item 4** (originally rank 2,
   ...
   With item 4 landed the interface has 2-3 real implementors (`TriangleArray3D`, `QuadArray3D`,
...
### Decision to make: dead interfaces carrying live derived bodies
...
family interfaces (`Curve2D/3D`, `Surface`, `Solid`, `PointSet*`...) inherit `Dimensioned` and
supply constant bodies per family in `geometry.library.plato`, or (b) retire the interface and
...
- **`Closed(x): Boolean` capability interface ("ClosureQueryable")** — DROPPED; the original
...
- **plato-320** (`PolygonMesh3D.UndirectedEdgeCount` closed-manifold guess) shares the
  `topology.interfaces` / `meshes-topology.library` seam: it is an independent correctness bug and
...
1. Land items 1-3, then item 4 followed by item 5, then the dead-interface decision — six small
...
2. Fold items 1-2 into the next P6/P8 library-body wave (risks scope drift; LIBRARIES.md rule 6
   requires interface extension to be its own justified change anyway).
...
generic algorithm exists** (`Field` has one; curves currently do not), and **no interface ships
without an implementor** (the `Dimensioned` half-state violates it; item 4's downgrade enforces
it prospectively). Seam: `curves.concepts.plato` / `meshes.concepts.plato` interface roots, which
...
- [x] `Curve<TRange>` added; Curve1D/2D/3D + PeriodicCurve re-rooted; at least the
      ...
      **Correction to the analysis above:** `curves.concepts.plato` was at **12/12**
      declarations, not 8/12 — the file was full, so the capability interfaces moved to a new
...
- [x] **DROPPED by decision, not deferred — `FaceIndexedGeometry3D<TFace>` should not be added.**
      ...
      interfaces collide — the tree already documents this at
      `meshes-geometry.library.plato:69-72`, where `IsEmpty` had to become `HasNoTriangles` for
      exactly that reason. So the generic interface would carry accessors but no count.
      ...
      Verdict: skip the new interface, populate `MeshIncidence` instead (next box). Note
...
- [x] `PrimitiveCollection3D<TPrimitive>` added on top of them; never landed before the
      ...
      and a zero-implementor interface is the failure mode this issue is closing.
...
- [x] **`MeshIncidence` populated** — `PolygonMesh3D` implements all six queries in the new
      ...
      `meshes-topology.library.plato:66-118` are reachable and the dead-interface class is fully
...
Item 1 alone: add the three-line `Curve<TRange>` interface, re-root four interfaces, collapse the
three `Sample` bodies. Gets the daily-paid duplication; gives up the mesh-access and doc wins
until later.

## Prevention

- A lint rule flagging interfaces with zero implementors AND derived library bodies would catch
...
- LIBRARIES.md rule 6 (grep `implements <Interface>` before extending) already prevents the
