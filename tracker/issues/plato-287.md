...
## Idea
**Is there a bounded SDF today?** Not as a first-class analytic concept. `SignedDistanceField2D/3D` are unbounded distance oracles; `SampledSdf2D/3D` carry a `Bounds` because the lattice has finite support, but there is no type/concept for "analytic (or procedural) SDF + conservative finite domain" used for culling, tiling, or CSG acceleration. Ask: should `BoundedSdf2D/3D` exist?
...
## Design decisions
- **Concept + wrapper** — `BoundedSignedDistanceField2D/3D` inherits SDF + `Bounded2D/3D`; concrete `BoundedSdf2D/3D` wraps `FunctionSdf*` + `Bounds`. `SampledSdf*` implements the bounded concepts.
...
- [stdlib/implicit-sdf-sampled.plato](../../submodules/Plato/stdlib/implicit-sdf-sampled.plato) — `SampledSdf*` implements bounded concepts
- [stdlib/geometry-kernels.library.plato](../../submodules/Plato/stdlib/geometry-kernels.library.plato) — `IntersectionOfBounds`

## Approaches
Short term: done (wrapper + concept + culling + CSG bound propagation).
...
- [x] If pursuing: `BoundedSdf2D/3D` (or concept) declared + lint clean
- [x] At least one operation uses Bounds for culling (even a trivial `Overlaps(a.Bounds, b.Bounds)` helper)
- [x] Semantics outside bounds written in the type/concept doc comment

## Simplest possible implementation
Shipped: concept + `BoundedSdf*` wrappers + SampledSdf implements + culling ops + CSG with bound propagation + `IntersectionOfBounds`.
