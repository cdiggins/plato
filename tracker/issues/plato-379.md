...
collection — under two names. Proposal: collapse them into one general concept parameterised
on the key, and make integer-indexed collections a named instance of it.

```
concept Indexable<TIndex, TValue> inherits Countable { At(x: Self, index: TIndex): TValue; }
concept ArrayLike<T> inherits Indexable<Integer, T> { }
```

`MapLike` is deleted. `ValueAt` disappears (its name folds into `At`). `ContainsKey` is not
carried into the general concept — key-membership is a separable capability that belongs to a
future `DictionaryLike`, in the same way `Contains` lives on `SetLike` rather than on
`Indexable`.

The mechanism this rests on is ordinary Plato concept inheritance with substituted type
...
exactly one `Eval`. Sub-concepts redeclare nothing. So `At(x: Self, Integer): T` on `Array`
...
- No concept in this stdlib uses a default body, so any inherited member is a real obligation
  on every implementer. This is why `ContainsKey` cannot ride along into the general concept
...
- **Where the ordering helpers live** — the eleven generic bodies in
  ...
  `Count` ordering. They cannot lift to the general concept; they must move to `ArrayLike<$T>`.
...
- **Where `ContainsKey` goes** — dropped now and reintroduced on a future `DictionaryLike`, vs
  kept on the general concept. Keeping it makes every array, vector and mesh owe a bounds test
...
- **Does this clear LINT013 honestly?** Under this shape arrays become transitive implementers
  of the general concept, so the `MapLike` finding disappears as a side effect. That is only
  legitimate because arrays genuinely *are* integer-keyed lookups — not because the concept was
...
- [plato-304](plato-304.md) — LinqArray port guidance; already states the repo noun is
  `Array` / concept `Indexable<$T>`. A rename to `ArrayLike` needs to be reflected there.
...
1. **Rename-only (option D)** — keep `MapLike` as the ancestor name, rename `ValueAt` → `At`,
   ...
   diff: the containers concepts file plus one helper. Gets the semantics with almost no churn,
...
3. **Do nothing** — leave both concepts and let plato-325 retire `MapLike` outright. Honest if
   no dictionary type is ever coming.

Long term: with the key parameterised, a `DictionaryLike` (adding `ContainsKey`, `Keys`,
`Values`) slots in beside `ArrayLike` with no further concept surgery, and the 2D/3D family can
...
unification exists to remove; (b) push `ContainsKey` onto the general concept to keep `MapLike`
...
- [ ] One lookup concept parameterised on the key; `ValueAt` no longer exists in
      `stdlib/foundation/`.
- [ ] `Array`, the vector concepts and `Face`/`TriMesh` each declare exactly one `At` and
...
- [ ] The eleven `collections-indexable.library.plato` helpers are re-headed onto the
      Integer-keyed concept and still resolve.
- [ ] `ContainsKey` / `DoesNotContainKey` either removed or relocated to a dictionary concept,
...
`concept Indexable<T> inherits Countable` to `inherits MapLike<Integer, T>` and drop its now
