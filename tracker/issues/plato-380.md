...
Spin-off from [plato-379](plato-379.md), deliberately kept separate. If the lookup concept
becomes parameterised on its key, then `Indexable2D<T>` and `Indexable3D<T>` are just that
concept at `Integer2` and `Integer3` keys, rather than separate concepts carrying their own
...
recommendation is to re-point `Indexable2D`/`3D` at the Integer-keyed concept and change nothing
else.
...
- Do `ColumnCount` / `RowCount` survive as concept members, or become properties of the key
...
[plato-379](plato-379.md) landing first — without a key-parameterised concept there is nothing
