...
## Issue
Agents and authors reviewing the concept lattice only have the flat
...
types — redundant concept-to-concept `inherits` clauses are invisible.
...
are refactored. ~162 forward concepts make a text forest practical.

## Affected code
- `src/Plato.Compiler/Analysis/Linter.cs:619` — `CheckRedundantImplements` / LINT011 (types only)
- `src/Plato.Compiler/Checking/ConceptClosure.cs` — transitive concept walk (checker)
...
`Inherits` on concepts; it was simply never written.
...
- Blocked by: none
- Blocks: none
- Touches: Linter, ContextExport, export script, `docs/concept-hierarchy.txt`
...
## Bedrock
Strengthens the **concept-lattice as a first-class reviewed artifact** seam: one graph
...
## Done means
- [x] LINT016 reports redundant concept `inherits` (Info), with a unit test
...
- [x] `Plato.ContextExport --hierarchy` writes the forest; script regenerates
      `docs/concept-hierarchy.txt` from forward `stdlib/`
